...
| **Legacy Appointments on Today** | REMOVED from Today (Beta Phase 3) | The old "Coming up" card read the prototype `appointments` domain (separate from the canonical Step 10 Calendar). It was removed from Today to avoid two competing "upcoming" surfaces; the canonical Calendar is the single source. (Beta Phase 6 then retired the legacy Appointments UI everywhere — see the Beta Phase 6 section below.) |
...
| **Log save truthfulness** | IMPLEMENTED (Beta Phase 5) | Logs apply optimistically for instant feedback. When SIGNED IN, that optimistic entry lives only in React state until the cloud write lands (it is NOT mirrored to localStorage — that mirror is the signed-out path). So a failed cloud write means the change is in memory only and will be lost on refresh/close. Instead of swallowing that failure (old `console.warn`) or falsely claiming it was "saved on this device", a truthful toast now says it couldn't sync to your household and may be lost / revert / reappear if you leave or refresh (wording per operation: add/edit/delete/sleep). It clears on the next successful write or load, and a stale write from a previous family/session can never surface it (session-generation guard). |
...
## Beta Phase 6 (Calendar & Appointments consolidation)

| Capability | Status | Reality today |
|---|---|---|
| **One scheduling source of truth** | IMPLEMENTED (Beta Phase 6) | The canonical Step 10 **shared Calendar** is the single home for anything that happens at a date or time. The older legacy Appointments UI is retired: a beta user no longer encounters both "Calendar" and "Appointments" as competing scheduling destinations. |
| **Legacy Appointments UI** | RETIRED (Beta Phase 6) | The legacy Appointments detail / composer / upcoming-list screens are removed and their overlays + nav entry points (`openAppointment` / `composeAppointment`, the `appointment`/`apptCompose`/`upcoming` overlays) are gone. The Me "Coming up" card now reads the canonical Calendar (next upcoming event → opens Calendar; "Add to the calendar" → the Calendar composer). No user-facing path creates a legacy appointment. |
| **Legacy appointment data** | DORMANT / PRESERVED (no migration) | Retiring the UI did NOT delete the `appointments` / `appointment_questions` tables or any existing rows, and no migration was run (chain stays at `0014`). Any historical beta rows are simply no longer surfaced through the app — they are not shown, not editable, and NOT converted into canonical Calendar events; they remain resident in the database (queryable only outside the retired beta UI). Two nuances: (1) the "Start over" reset still wipes a family's appointment rows along with the rest of its data (`clearFamilyData` deletes `appointment_questions` + `appointments`, unchanged by Phase 6), and `clearAppointments()` clears the local copy — so reset is thorough, not a leak; (2) the retained `AppointmentsProvider` still *reads* those tables on load (see TECHNICAL_DEBT) but nothing renders the result. A future cleanup could drop the provider + tables (migration), but that is out of scope. |
| **Appointment preparation ≠ scheduling** | PRESERVED (distinct) | "Questions for my doctor" (Mom `mom_items`) and First-90 "Save a question for my doctor" stay a separate preparation mechanism — they are NOT Calendar fields and were not folded into scheduling. A pediatrician visit is simply a Calendar event; the questions to ask at it live in Mom. |
| **Today scheduling** | CANONICAL (unchanged) | Today derives schedule from the canonical Calendar only (the legacy appointment card was removed in Phase 3 and did not return). |
| **Tell scheduling** | CANONICAL (unchanged) | Tell proposes only `CALENDAR_CREATE` (never a legacy appointment), requires confirmation, and executes through the trusted Calendar RPC. |
| **Recurrence** | NOT IMPLEMENTED | Repeating events (e.g. weekly soccer) are not supported; each event is one-time. Future work. |
