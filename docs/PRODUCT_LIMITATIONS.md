...
| **Legacy appointment data** | DORMANT / PRESERVED (no migration) | The `appointments` / `appointment_questions` tables and any existing rows are NOT deleted — hiding the UI strands nothing destructively, and no migration was run (chain stays at `0014`). Any historical beta rows simply aren't surfaced; the data remains queryable in the database. The `AppointmentsProvider` is retained so the reset flow still clears local appointment data. A future cleanup could migrate/drop it, but that is out of scope. |
